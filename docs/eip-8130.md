...
- **Recent merges (early June 2026)**: a finalization burst landed nine PRs in five days (Jun 1 – Jun 5): #11752 (signature-invariant nonce-free replay identifier), #11757 (constants / gas accounting / interface cleanups), #11764 (rename `owners` → `actors` and finalize Transaction Context naming), #11765 (rename `IAccountConfiguration.verify` → `verifyActor`), #11766 (add actor call policies and per-actor expiry), #11767 (clean up, reorganize and finalize pass), #11769 (fixes and clarifications). PR #11751 (delegated account creation) remains open. The naming refactor (`owners` → `actors`) is the largest semantic shift and signals the spec is approaching a frozen surface for Hegotá inclusion.
