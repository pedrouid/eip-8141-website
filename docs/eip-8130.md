...
| **Signature schemes** | Deploy authenticator contract, add to canonical set | Write account code that calls `APPROVE` |
...
- **Recent merges (June-July 2026)**: a sustained finalization burst landed across Jun 1 – Jul 8. The first wave (Jun 1 – Jun 10) shipped #11752, #11757, #11764 (`owners` -> `actors`), #11765, #11766, #11767, #11769, #11782, #11785 (`verifier` -> `authenticator`), #11791, and #11794. PR #11751 (delegated account creation) closed Jun 18 without merge. The July wave added #11847 (actor-policy clarification and opaque metadata simplification, merged Jul 2) and #11903 (renumber `AA_TX_TYPE` to `0x79`, `AA_PAYER_TYPE` to `0x7A`, and add `eth_call`/`eth_estimateGas` support for AA transaction fields, merged Jul 8). The terminology and RPC surface are now much closer to implementation-ready.
