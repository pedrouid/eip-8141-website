...
- Custom signature witnesses use `ARBITRARY` entries and cost 100 gas each. `SIGPARAM` exposes their length and `SIGDATACOPY` copies their bytes after PR #12187, so compilers and simulators no longer handle dynamic `SIGPARAM` stack arity.
- Wallets must estimate two budgets for every frame: `limits.execution` and `limits.state`. They cannot borrow across dimensions or frames; under-provisioning either halts the frame, while over-provisioning raises payer escrow. Data-heavy transactions pay the calldata floor plus net state gas (PR #12062).
- Gas estimators must use the pinned initial warm set from PR #12113: sender, coinbase, and active precompiles start warm; frame targets do not warm by position; the payer warms only when `APPROVE` touches it.
- Frame transactions support blobs through the EIP-7594 pooled-transaction wrapper. Wallet fee estimation must reserve `max_gas` plus blob gas and display the final `payer_refund`; every fee field must be below `2**256`.
...
- **Receipts and fees**: per-frame receipts expose `[execution, state]` usage, while the transaction receipt has no single status. Later state refills may reduce an earlier frame's state usage. Wallets should derive any aggregate status, use transaction `gas_used` for final cost, and use frame receipts for diagnostics (#12061-#12062).
...
A first datapoint on the direction of travel: ERC-8286 (chiranjeev13, [ERC PR #1794](https://github.com/ethereum/ERCs/pull/1794), merged Jul 28 2026) standardizes how [ERC-7579](https://eips.ethereum.org/EIPS/eip-7579) modular accounts (validator, executor, hook, and config modules) implement the EIP-8141 validation flow: a validator module returns an approval mode the account applies via `APPROVE` inside a VERIFY frame. It is the first merged ERC built on top of EIP-8141 (`requires: 7579, 8141`), and it targets exactly the permissions and session-key layer that protocol defaults leave open. Its merge turns the July AllWalletDevs comparison with EIP-8130 into a concrete downstream standard rather than a repository signal.
