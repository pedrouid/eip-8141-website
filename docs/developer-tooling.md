...
- For gas sponsorship, prefer the canonical paymaster for the public-mempool path (ETH-funded only). Use EOA-as-paymaster only when the index-0 default-code signature rule does not conflict with sender validation.
