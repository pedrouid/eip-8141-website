...
EIP-8141 decouples PQ scheme selection from the transaction format: as precompiles ship, accounts adopt them by updating VERIFY frame logic, no protocol changes needed. The [signatures list](/merged-changes) (PR [#11481](https://github.com/ethereum/EIPs/pull/11481), merged May 22; repaired by #11837/#11814 in July) adds forward-compatibility with signature aggregation, critical since PQ signatures are large (Falcon: ~666 bytes, Dilithium: ~2,420 bytes). Protocol-validated raw bytes remain hidden from EVM code for aggregation, while `ARBITRARY` entries cost 100 gas and carry custom witnesses. PR #12187 split access into static-shape operations: `SIGPARAM` returns witness length and `SIGDATACOPY` copies bytes. Raw signature bytes with empty `msg` are elided from the canonical hash. The proposed explicit registry in PR #12011 closed Jul 29; future fixed-cost schemes remain separate specification work.

PR [#11772](https://github.com/ethereum/EIPs/pull/11772) (proposed as EIP-8288, opened Jun 5 by vbuterin and Thomas Coratger) makes block-level aggregation concrete and remains open as of Aug 20. It introduces a new frame mode `DEP_VERIFY_FRAME_MODE = 3` that declares `(scheme, data_hash, verification_key)` dependencies and a top-level block-header field `recursive_stark` that aggregates every per-block dependency into a single recursive STARK proof using Lean Ethereum tooling (`LEANSPHINCS_SCHEME = 0x10` at 3000 gas, `LEANSTARK_SCHEME = 0x11` at 30000 gas). Review still covers recursive knowledge soundness. August forum discussion added a distinct completeness question: a proof can establish validity of the included dependency set without proving that the set contains every admitted dependency. EIP-8288 is intended to be FOCIL-compatible by construction, but both properties must be explicit.
